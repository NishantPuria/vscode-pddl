...
export class MemFS implements vscode.FileSystemProvider {
    ...
    deleteRecursive(uri: vscode.Uri): void {
        for (const [name, fileType] of this.readDirectory(uri)) {
            let fileUri = uri.with({path: uri.path + '/' + name});
            if (fileType && vscode.FileType.Directory == vscode.FileType.Directory){
                this.deleteRecursive(fileUri);
            }
            this.delete(fileUri);
        }
    }
